...
/* printing of commands for troubleshooting */
    ...
    case EMCMOT_DIRECT_POLYLINE: {
		...
		int res_addDirectPoint = 0;

		int i = 0;
		for (i = 0; i < emcmotCommand->num_points; ++i) {
			res_addDirectPoint = (res_addDirectPoint || emcmotConfig->vtp->tpAddDirectPoint(&emcmotDebug->tp,
							   emcmotCommand->points[i],
							   emcmotCommand->motion_type,
							   emcmotCommand->vel,
							   emcmotCommand->ini_maxvel,
							   emcmotCommand->acc,
		...
		if (res_addDirectPoint != 0) {
			reportError(_("can't add direct polyline move at line %d, error code %d"),
					emcmotCommand->id, res_addDirectPoint);
			emcmotStatus->commandStatus = EMCMOT_COMMAND_BAD_EXEC;
			emcmotConfig->vtp->tpAbort(&emcmotDebug->tp);
			SET_MOTION_ERROR_FLAG(1);
