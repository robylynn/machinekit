...
static void process_inputs(void) {
	...
	for (joint_num = 0; joint_num < num_joints; joint_num++) {
		...
		if (abs_ferror > joint->ferror_limit) {
			// msati3: Don't compute following error at the moment
	...
	for (joint_num = 0; joint_num < num_joints; joint_num++) {
		...
		*(joint_data->home_state) = joint->home_state;
		...
		if (joint_num < 9) {
			...
			log_joint_pos_fb[joint_num] = *joint_data->joint_pos_fb;
		}
	} // while joint end
	if ((*(emcmot_hal_data->program_line) != 0)
			& (*(emcmot_hal_data->requested_vel) != 0)) {
		syslog_async(LOG_ERR, "*MUKUL JOINT* %i %f %f %f %f %f %f",
				*(emcmot_hal_data->program_line), log_joint_pos[0],
				log_joint_vel[0], log_joint_pos_fb[0], log_joint_pos[1],
				log_joint_vel[1], log_joint_pos_fb[1]);
	}
}
