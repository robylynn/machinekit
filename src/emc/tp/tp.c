...
/** static function primitives (ugly but less of a pain than moving code around)*/STATIC int tpComputeBlendVelocity(
...
STATIC int tpCreateArcLineBlend(TP_STRUCT * const tp, TC_STRUCT * const prev_tc,
	...
	int coplanar = pmCartCartParallel(&geom.binormal,
			&prev_tc->coords.circle.xyz.normal, TP_ANGLE_EPSILON);
...
/** Wrapper function to unlock rotary axes */STATIC void tpSetRotaryUnlock(
		TP_STRUCT * const tp, int axis, int unlock) {
	SetRotaryUnlock(tp->shared, axis, unlock);
}

/** Wrapper function to check rotary axis lock */STATIC int tpGetRotaryIsUnlocked(
		TP_STRUCT * const tp, int axis) {
 ...
 */STATIC int tpCheckEndCondition(TP_STRUCT const * const tp,
		TC_STRUCT * const tc, TC_STRUCT const * const nexttc) {
